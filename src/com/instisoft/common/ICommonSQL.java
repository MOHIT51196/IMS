...
public interface ICommonSQL {
	...
	final String READ_FACULTY_LIST_SQL = "SELECT * FROM faculty_master f, faculty_batch_mapping fb,  batches b, faculty_address_mapping fa, address a WHERE f.name=? and f.fid = fb.fid and fb.bid = b.bid";
	final String READ_FACULTY$BATCH_SQL = "SELECT * FROM faculty_master f,  batches b, faculty_batch_mapping fb WHERE f.name=? and f.fid = fb.fid and fb.bid = b.bid)";
	final String FIND_FACULTY_SQL = "SELECT * FROM faculty_master WHERE f.name=?";
	...
	final String ADD_STUDENT_SQL = "INSERT INTO student_master(sid,name,gender,dob,pemail,semail,pphone,sphone,fname,femail,fphone,mname,memail,mphone,doj) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	final String READ_COURSE_LIST_SQL = "SELECT * FROM courses c, course_batch_mapping cb, batches b, course_faculty_mapping cf, faculty f WHERE c.cid=cb.cid and cb.bid=b.bid and c.cid=cf.cid and cf.fid=f.fid";
	final String FIND_COURSE_SQL = "SELECT * FROM courses WHERE name=?";
	final String FIND_COURSE$BATCH_SQL = "SELECT * FROM courses c, course_batch_mapping cb, batches b WHERE c.name=? and c.cid = cb.cid and cb.bid = b.bid";
	final String FIND_COURSE$FACULTY_SQL = "SELECT * FROM courses c, course_faculty_mapping cf, faculty b WHERE c.name=? and c.cid = cf.cid and cf.fid = b.fid";

	
	final String ADD_BATCH_SQL = "INSERT INTO batches(bid,name,cid,category,startdate,enddate,time,hour) VALUES(?,?,?,?,?,?,?,?)";
	
	final String FIND_BATCH_SQL = "SELECT * FROM batches WHERE name=?";
	final String READ_BATCH_LIST_SQL = "SELECT * FROM batches b, courses c, course_batch_mapping cb WHERE b.bid = cb.bid and cb.cid = c.cid";
